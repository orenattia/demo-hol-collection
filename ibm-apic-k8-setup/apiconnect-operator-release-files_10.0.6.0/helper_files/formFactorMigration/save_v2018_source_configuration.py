...
def createManagementBackup():
    ...
    if args.skip_csv_download == False:
        ...
        if downloadUsingNginxImage == False:
            ...
            a1 = runKubernetesCommand(cpCommand, None, True, 0, False)
            c1 = 0
            while a1 != None and "No such file or directory" in a1:
                print ("Waiting for extract completion :", time.asctime())
                time.sleep(120)
                a1 = runKubernetesCommand(cpCommand, None, True, 0, False)
                c1 = c1 + 1
                if c1 > 60:
                    print("Exiting loop as waited for 2 hours for extract to complete")
                    break;
        ...
        print ("Attach the pvc to a pod using mount path /upgrade and download the contents. PVC name : ", pvc_name)
        print ()
        print ("Directory structure after download, check it :: ")
